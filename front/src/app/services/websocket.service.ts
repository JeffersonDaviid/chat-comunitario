...
import { wsBaseUrl } from '../conf/global'
...
export class WebsocketService implements OnDestroy {
	...
	private async joinChannel(params: WSConnectParams): Promise<void> {
		...
		try {
			...
			console.log(
				`[SignalR] Unido al canal ${params.channelId} en comunidad ${params.communityId}`,
			)
		} catch (err) {
	...
	private async sendMessage(
		...
		if (state !== signalR.HubConnectionState.Connected) {
			console.error(
				`[SignalR] No conectado (estado: ${state}). No se puede enviar el mensaje.`,
			)
