...
import { apiBaseUrl, baseFiles } from 'src/app/conf/global'
...
export class ChatComponent implements OnInit, OnDestroy, AfterViewChecked, OnChanges {
	...
	ngOnInit(): void {
		...
		console.log(
			`[Chat] Componente iniciado - Community: ${this.communityId}, Channel: ${this.channelId}`,
		)
	...
	ngOnChanges(changes: SimpleChanges): void {
		...
		if (
			(changes['communityId'] || changes['channelId']) &&
			!changes['communityId']?.firstChange
		) {
	...
	private initializeChat(): void {
		...
		if (
			this.communityId &&
			this.channelId &&
			(!this.communityName || !this.channelName)
		) {
			this.http.get<any>(`${apiBaseUrl}/community/${this.communityId}`).subscribe({
				next: (res) => {
					...
					console.log(
						`[Chat] Nombres obtenidos - Comunidad: ${this.communityName}, Canal: ${this.channelName}`,
					)
	...
	private connectAndLoadMessages(): void {
		...
		this.subMsg = this.ws.messages$().subscribe((msg) => {
			console.log(
				'[Chat] Mensaje recibido del WS:',
				msg.type,
				msg.payload?.message || msg.payload?.text || msg.payload?.content,
			)

			// Si es un mensaje de bienvenida, extraer nombres de comunidad y canal
			if (
				msg.type === 'welcome' &&
				msg.payload?.communityName &&
				msg.payload?.channelName
			) {
				this.communityName = msg.payload.communityName
				this.channelName = msg.payload.channelName
				console.log(
					`[Chat] Nombres recibidos del WS - Comunidad: ${this.communityName}, Canal: ${this.channelName}`,
				)
			...
			if (payloadChannel && payloadChannel !== this.channelId) {
				console.log(
					`[Chat] Mensaje descartado: no pertenece a este canal (${payloadChannel} !== ${this.channelId})`,
				)
				return
			}
		...
		if (this.communityId && this.cedula && this.channelId) {
			console.log(
				`[Chat] Llamando setIdentity - comunityId: ${this.communityId}, channelId: ${this.channelId}`,
			)
			this.ws.setIdentity(
				{
				...
				true,
			) // reconnect = true para establecer la conexión inmediatamente
		}
	}
	...
	private loadUserDataFromService() {
		...
		if (u) {
			this.myFullName =
				[u?.name, u?.lastName].filter(Boolean).join(' ').trim() || u?.email || 'Usuario'
	...
	loadHistory() {
		...
		this.http
			.get<any>(
				`${apiBaseUrl}/message/channel/${this.communityId}/${this.channelId}?limit=50`,
			)
			.subscribe({
				next: (res) => {
					...
					console.log(
						`[Chat] ✅ Historial cargado. Total de mensajes: ${this.messages.length}`,
					)
				},
				error: (err) => {
	...
	send() {
		...
		if (this.filePreview) {
			...
			reader.onload = (e) => {
				...
				this.ws.sendChannelMessage(this.channelId, messagePayload).catch((err) => {
	...
	onFileSelected(event: any) {
		...
		if (!isImage && !isDocument) {
			alert(
				'Tipo de archivo no permitido. Use imágenes (JPG, PNG) o documentos (PDF). Máximo: 50MB',
			)
			return
		}
	...
	downloadFile(m: WSMessage) {
		...
		// Caso 2: archivo como URL del servidor - usar API de descarga
			...
			fileUrl = `${apiBaseUrl}/files/chat/${guidFileName}`
		}
	...
	getFileUrl(m: WSMessage): string {
		...
		if (p.file && typeof p.file === 'string') {
			return p.file.startsWith('http') ? p.file : `${baseFiles}${p.file}`
		}
		// Caso 3: estructura anidada
		if (p.payload?.file) {
			const nested = p.payload.file
			if (typeof nested === 'object' && nested.data) {
				...
				return nested.startsWith('http') ? nested : `${baseFiles}${nested}`
			}
		}
	...
	getUserName(m: WSMessage): string {
		...
		if (msgCedula) return `${String(msgCedula)}`
	...
	getUserAvatar(m: WSMessage): string {
		...
		const msgCedula = p.cedula

		return apiBaseUrl + '/images/profile/' + msgCedula
	}
